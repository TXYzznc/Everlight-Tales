...
```
【主体与画面需求】
一个静默区异常标记，俯视视角，静默抑制场剪影。抑制场沿边界切分、波纹沿抑制分面。静默抑制场，品红紫系，抑制处以深色点提。白色背景，居中构图，游戏道具概念设计。
...
【笔触、质感与材料】
以高覆盖度、平整而有细微手工变化的数字色块为基础，辅以少量顺形体方向的短笔触与局部色面叠压。静默抑制场通过边界折线与受抑波纹表现；不依赖高清照片贴图、微观孔隙、全画面颗粒噪声或随机碎裂笔触。整体干净、精炼、具有适度的手绘设计感。
...
```
A silence zone anomaly marker, game prop, top-down view, silencing suppression field silhouette. Field faceted along the boundary, ripples planar-shaded as suppressed. Silencing suppression field, magenta-purple color group, suppression picked out in dark tone. White background, centered composition, game prop concept design.
